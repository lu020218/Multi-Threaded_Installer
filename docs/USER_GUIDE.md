...
## Appendix: `defaultSelected` and UI checkbox binding

- `components[].defaultSelected` only controls the default selected state of a component.
- It does not bind to a UI checkbox by `id` alone.
- The checkbox state is updated only after component-to-UI binding is established.
- Required conditions:
  - `ui.componentSelection.mode` is `embeddedInExistingPages` or `hybrid`
  - `ui.componentSelection.binding.pages[].controls` contains the checkbox `name`
  - the XML `CheckBox` has `userdata="component:<componentId>"`
- Example:
...
- If `userdata` is missing, the checkbox name is not listed in `controls`, or you are not running the final installer generated by `packager.exe`, the UI will not reflect `defaultSelected`.
