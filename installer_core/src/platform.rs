...
mod windows_impl {
    ...
    impl Platform for WindowsPlatform {
        ...
        fn create_shortcut(&self, name: &str, target: &Path, icon: Option<&Path>) -> Result<()> {
            ...
            let mut lnk = mslnk::ShellLink::new(target)
                .map_err(|e| InstallerError::Platform(format!("Failed to create shortcut: {}", e)))?;
            ...
            lnk.create_lnk(&shortcut_path)
                .map_err(|e| InstallerError::Platform(format!("Failed to write shortcut: {}", e)))?;
        ...
        fn register_uninstaller(&self, info: &UninstallInfo) -> Result<()> {
            ...
            let (key, _) = hklm.create_subkey(&path)
                .map_err(|e| InstallerError::Platform(format!("Failed to create registry key: {}", e)))?;
            
            key.set_value("DisplayName", &info.app_name)
                .map_err(|e| InstallerError::Platform(format!("Failed to set DisplayName: {}", e)))?;
            key.set_value("DisplayVersion", &info.version)
                .map_err(|e| InstallerError::Platform(format!("Failed to set DisplayVersion: {}", e)))?;
            key.set_value("InstallLocation", &info.install_location.to_string_lossy().to_string())
                .map_err(|e| InstallerError::Platform(format!("Failed to set InstallLocation: {}", e)))?;
            key.set_value("UninstallString", &info.uninstall_exe.to_string_lossy().to_string())
                .map_err(|e| InstallerError::Platform(format!("Failed to set UninstallString: {}", e)))?;
            key.set_value("EstimatedSize", &(info.estimated_size_kb as u32))
                .map_err(|e| InstallerError::Platform(format!("Failed to set EstimatedSize: {}", e)))?;
            
            if let Some(ref publisher) = info.publisher {
                key.set_value("Publisher", publisher)
                    .map_err(|e| InstallerError::Platform(format!("Failed to set Publisher: {}", e)))?;
            }
        ...
        fn read_registry(&self, path: &str, key: &str) -> Result<String> {
            ...
            let subkey = root_key.open_subkey(subpath)
                .map_err(|e| InstallerError::Platform(format!("Failed to open registry key: {}", e)))?;
            
            subkey.get_value(key)
                .map_err(|e| InstallerError::Platform(format!("Failed to read registry value: {}", e)))
        }
        
        fn write_registry(&self, entry: &RegistryEntry) -> Result<()> {
            debug!("Writing registry: {}\\{} = {}", entry.path, entry.key, entry.value);
            
            let (root, subpath) = parse_registry_path(&entry.path)?;
            let root_key = RegKey::predef(root);
            
            let (key, _) = root_key.create_subkey(subpath)
                .map_err(|e| InstallerError::Platform(format!("Failed to create registry key: {}", e)))?;
            
            match entry.value_type {
                RegistryValueType::String => {
                    key.set_value(&entry.key, &entry.value)
                        .map_err(|e| InstallerError::Platform(format!("Failed to set string value: {}", e)))?;
                }
                RegistryValueType::Dword => {
                    let dword_value: u32 = entry.value.parse()
                        .map_err(|_| InstallerError::Platform(format!("Invalid DWORD value: {}", entry.value)))?;
                    key.set_value(&entry.key, &dword_value)
                        .map_err(|e| InstallerError::Platform(format!("Failed to set DWORD value: {}", e)))?;
                }
                RegistryValueType::ExpandString => {
                    key.set_value(&entry.key, &entry.value)
                        .map_err(|e| InstallerError::Platform(format!("Failed to set expand string value: {}", e)))?;
                }
            }
        ...
        fn request_elevation(&self) -> Result<()> {
            ...
            let exe_path = std::env::current_exe()
                .map_err(|e| InstallerError::Platform(format!("Failed to get current exe: {}", e)))?;
            ...
            let status = std::process::Command::new("cmd")
                .args(["/C", "start", "", "/wait", "runas", &exe_path.to_string_lossy(), &args_str])
                .status()
                .map_err(|e| InstallerError::Platform(format!("Failed to request elevation: {}", e)))?;
        ...
        fn is_process_running(&self, name: &str) -> Result<bool> {
            ...
            let is_running = system.processes().values().any(|process| {
                process.name().to_string_lossy().to_lowercase() == name_lower
            });
        ...
        fn configure_auto_startup(&self, app_name: &str, exe_path: &Path, enable: bool) -> Result<()> {
            ...
            if enable {
                ...
                key.set_value(app_name, &exe_path.to_string_lossy().to_string())
                    .map_err(|e| InstallerError::Platform(format!("Failed to set auto-startup: {}", e)))?;
            } else {
        ...
        fn check_windows_version(&self, min_version: &WindowsVersion) -> Result<bool> {
            ...
            if !meets_requirement {
                info!(
                    ...
                    min_version.major, min_version.minor, min_version.build
                );
            }
    ...
    impl WindowsPlatform {
        ...
        fn get_version_from_registry(&self) -> Result<WindowsVersion> {
            ...
            let key = hklm.open_subkey("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion")
                .map_err(|e| InstallerError::Platform(format!("Failed to open version key: {}", e)))?;
            ...
            let build_str: String = key.get_value("CurrentBuildNumber").unwrap_or_else(|_| "19041".to_string());
            ...
            debug!("Windows version: {}.{}.{}", version.major, version.minor, version.build);
            Ok(version)
        }
    }
...
mod stub_impl {
    ...
    impl Platform for WindowsPlatform {
        ...
        fn create_shortcut(&self, _name: &str, _target: &Path, _icon: Option<&Path>) -> Result<()> {
            Err(InstallerError::Platform("Not supported on this platform".to_string()))
        }
        ...
        fn read_registry(&self, _path: &str, _key: &str) -> Result<String> {
            Err(InstallerError::Platform("Registry not available on this platform".to_string()))
        }
        ...
        fn configure_auto_startup(&self, _app_name: &str, _exe_path: &Path, _enable: bool) -> Result<()> {
            Err(InstallerError::Platform("Not supported on this platform".to_string()))
        }
...
mod tests {
    ...
    fn test_check_windows_version() {
        ...
        let win7 = WindowsVersion { major: 6, minor: 1, build: 0 };
        assert!(platform.check_windows_version(&win7).unwrap());
        
        // Should fail for future Windows version
        let future = WindowsVersion { major: 99, minor: 0, build: 0 };
