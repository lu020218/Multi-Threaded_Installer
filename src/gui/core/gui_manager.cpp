...
GUIManager::~GUIManager() {
    ...
    // m_pWorker / m_pUninstallWorker / m_pPageController 为 unique_ptr，随对象析构自动释放。
}
...
void GUIManager::InitWindow() {
    ...
    if (m_pTabPages && !m_uninstallMode) {
        m_pPageController = std::make_unique<PageController>(m_pTabPages);
    }
...
bool GUIManager::StartInstallationWithOptions(const std::wstring& installPath,
    ...
    if (!m_pWorker) {
        m_pWorker = std::make_unique<InstallationWorker>(m_hWnd);
    }
...
void GUIManager::OnUninstallConfirmClick() {
    ...
    if (!m_pUninstallWorker) {
        m_pUninstallWorker = std::make_unique<UninstallWorker>(m_hWnd);
    }
