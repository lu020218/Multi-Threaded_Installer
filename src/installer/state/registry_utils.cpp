...
bool deleteMatchingUninstallRegistryEntries(const std::string& installDir,
    ...
    const wchar_t* uninstallSubkeyW = L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall";
    UniqueHKey uninstallKey;
    LONG status = RegOpenKeyExW(root, uninstallSubkeyW, 0, KEY_ENUMERATE_SUB_KEYS | KEY_QUERY_VALUE, uninstallKey.receive());
    if (status != ERROR_SUCCESS) {
        return false;
    }
    ...
    while (RegEnumKeyExW(uninstallKey.get(), index, nameBuffer, &nameLen, nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS) {
        ...
        if (RegOpenKeyExW(root, (std::wstring(uninstallSubkeyW) + L"\\" + subkeyNameW).c_str(),
            ...
            auto queryString = [&](const wchar_t* valueName) -> std::string {
                DWORD type = 0;
                DWORD size = 0;
                LONG queryStatus = RegQueryValueExW(entryKey.get(), valueName, nullptr, &type, nullptr, &size);
                if (queryStatus != ERROR_SUCCESS || (type != REG_SZ && type != REG_EXPAND_SZ) || size == 0) {
                    return {};
                }
                std::wstring value;
                value.resize(size / sizeof(wchar_t));
                queryStatus = RegQueryValueExW(entryKey.get(), valueName, nullptr, &type,
                                               reinterpret_cast<BYTE*>(&value[0]), &size);
